...
#include "sensors.h"
#include "error.h"
// todo : gitlab ci, apt install libsensors-dev
...
{
public:
    libsensors_handle(std::string config_path = {})
...
struct _sensors_impl<feature>::impl : public impl_base<sensors_feature>
{
    ...
    impl static find(std::string chip_path, std::string feature_name)
    {
        int nr = 0;
...
struct _sensors_impl<subfeature>::impl : public impl_base<sensors_subfeature>
{
    ...
    impl static find(std::string full_path)
    {
        filesystem::path path {full_path};
...
// sensors::chip_name
//
chip_name::chip_name(std::string path)
...
std::string chip_name::name() const
{
    ...
    auto const written = sensors_snprintf_chip_name(const_cast<char*>(name.c_str()), size, **this);
    if (written < 0)
...
// sensors::feature
//
feature::feature(std::string full_path)
    : feature{full_path, filesystem::path{full_path}.filename().string()}
{}

feature::feature(std::string chip_path, std::string feature_name)
...
#ifndef STD_OPTIONAL_FOUND
std::experimental::optional
#else
std::optional
#endif
<subfeature> feature::subfeature(subfeature_type type) const
{
...
// sensors::subfeature
//
subfeature::subfeature(std::string path)
